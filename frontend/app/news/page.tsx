...
import { fetchPortfolio, fetchNewsSentiment, fetchAiSignal, toggleBot, setStrategy } from "@/services/api";
...
export default function NewsPage() {
  ...
  const signalQuery = useQuery({ queryKey: ["ai-signal", "BTC/USDT"], queryFn: () => fetchAiSignal("BTC/USDT"), refetchInterval: 15000 });
      ...
      <div className={`flex-1 min-w-0 p-4 transition-all duration-300 lg:p-6 ${sidebarCollapsed ? "ml-20" : "ml-64"}`}>
        ...
        <main className="space-y-6">
          <div className="flex items-center justify-between">
            <h1 className="text-xl font-bold tracking-tight text-slate-100">Live Crypto News & Sentiment Feed</h1>
          </div>
