...
class SpreadDetector:
    """Calculates Net Executable Arbitrage Spread with Full Friction Deductions.
    ...
    def compute_spread(
        ...
        if quote_status == "DATA_STALE" or data_age_ms > self.MAX_QUOTE_AGE_MS:
            return ArbitrageSpread(
                symbol=symbol,
                buy_exchange=buy_exchange,
                sell_exchange=sell_exchange,
                buy_price=round(buy_ask_price, 2),
                sell_price=round(sell_bid_price, 2),
                gross_spread_usd=round(sell_bid_price - buy_ask_price, 2),
                gross_spread_pct=round(((sell_bid_price - buy_ask_price) / buy_ask_price) * 100.0, 4),
                total_fees_bps=buy_fee_bps + sell_fee_bps,
                transfer_cost_usd=1.0,
                latency_penalty_bps=0.0,
                slippage_bps=slippage_bps,
        ...
        transfer_cost_bps = (transfer_cost_usd / (buy_ask_price * 0.1)) * 10000.0 if buy_ask_price > 0 else 0.0
